...
const formatAskPrompt = ({ DBQueryResult, queryText }) => {
  ...
  4. Please DON'T ANSWER THAT IS NOT RELATED TO MY INFORMATION. Please reject the question gracefully in Korean.
...
const promptRAGClient = async (targetPrompt, sendMessage) => {
  ...
  const finalAnswerQuestion = await model.generateContentStream([formatAskPrompt({ DBQueryResult: result, queryText: targetPrompt })]);

  for await (const item of finalAnswerQuestion.stream) {
    sendMessage(item.candidates[0].content.parts[0].text); // Send the result via WebSocket
  }
  sendMessage(null, true); // Indicate that streaming is done
}

export default promptRAGClient

// getRAGClient('당신의 이름은 무엇입니까?')